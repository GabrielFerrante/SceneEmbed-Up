...
def visualizar_e_salvar_grafo(caminho_json: str) -> Data:
    """
    Carrega um JSON de resultados e gera visualização do grafo em PNG.

    Parameters
    ----------
    caminho_json:
        Caminho para o arquivo JSON gerado pelo pipeline de avaliação.

    Returns
    -------
    torch_geometric.data.Data
        Estrutura de grafo equivalente em PyG (apenas `edge_index`).
    """
    ...
    plt.title(
        f"Visualização de Grafos | Cobertura: {coverage:.2%}\nArquivo: {os.path.basename(caminho_json)}"
    )
