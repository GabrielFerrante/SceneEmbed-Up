"""
eval_retrieval_pca_h5.py
-------------------------
Avaliacao de retrieval (Recall@K bidirecional, I2T e T2I) do
LoRACrossAttentionAligner treinado com selecao de patches via PCA
(PCAVarianceSampler), sobre shards HDF5 anyup (1024 patches, 32x32).

Diferente do eval_retrieval_ats_h5.py, o checkpoint contem apenas o aligner
(PCAVarianceSampler e parametro-free, nao e salvo). Para cada batch:
  1. PCA seleciona K=196 patches de maior variancia dos 1024 patches AnyUp
  2. Aligner roda cross-attention sobre os patches selecionados
"""
...
# ---------------------------------------------------------------------------
# Avaliador PCA + Aligner
# ---------------------------------------------------------------------------

class PCARetrievalEvaluator:
    """Avaliador de retrieval (Recall@K) com selecao de patches por variancia PCA."""

    def __init__(
        self,
        sampler: PCAVarianceSampler,
    ...
    def evaluate_projection(
        ...
        selected     : [B, 196, 768]   — saida do PCAVarianceSampler
        attn_output  : [B, 1, 4096]    — saida do aligner (dim do texto)
        v_global     : [B, 4096]       — embedding visual refinado, pos-squeeze
        V            : [N, 4096]       — todos os visuais, CPU
        T            : [N, 4096]       — todos os textos, CPU
        sim_matrix   : [N, N]          — similaridades simetricas
        """
        self.aligner.eval()
        ...
        print("1. Extracting visual (PCA-selected) and text representations...")
        for visual_input, text_queries in tqdm(dataloader, desc="Embedding extraction"):
            # PCAVarianceSampler opera em float32 (estabilidade do SVD)
            selected, _, _ = self.sampler(visual_input)                  # [B, 196, 768]

            visual_input = visual_input.to(self.device).to(self.dtype)
            text_queries = text_queries.to(self.device).to(self.dtype)
            selected     = selected.to(self.device).to(self.dtype)       # [B, 196, 768]

            attn_output, _, _ = self.aligner(selected, text_queries)
            v_global = attn_output.squeeze(1)                            # [B, 4096]

            # Normalizacao L2 — garante produto escalar = cos sim
        ...
        assert V.shape[1] == T.shape[1], (
            f"Dimensoes incompativeis: V={V.shape}, T={T.shape}."
        )

        N = V.shape[0]
        print(f"2. Building symmetric similarity matrix [{N}x{N}]...")
...
if __name__ == "__main__":
    ...
    # PCAVarianceSampler e parametro-free: nenhum peso a carregar
    ...
    weights_path = "checkpoints/best_pca_aligner.pth"
    if os.path.exists(weights_path):
        aligner.load_state_dict(torch.load(weights_path, map_location=device))
        print(f"Pesos carregados de {weights_path}")
    else:
        print("Checkpoint not found. Running with random weights.")

    aligner = aligner.to(device).to(torch.bfloat16).eval()
    ...
    evaluator = PCARetrievalEvaluator(sampler=sampler, aligner=aligner, device=device)

    print("\n--- Avaliando Alinhamento PCA (Recall@K Bidirecional) ---")
    ...
    salvar_recall_results(recall_results, path="results/recall_metrics_pca.json")
